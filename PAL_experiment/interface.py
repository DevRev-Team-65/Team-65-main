...
SYSTEM_MESSAGES = 'You generate JSON schemas for given queries.'
class ProgramChatInterface(ProgramInterface):
    def __init__(self, *args, system_message: str = SYSTEM_MESSAGES, **kwargs):
        super().__init__(*args, **kwargs)
        self.system_message = system_message
        
    def generate(self, prompt: str, temperature: float = 0.0, top_p: float = 1.0, max_tokens: int = 512):
        messages = [{'role': 'system', 'content': 'You are a helpful chatbot who provides JSON schemas.'}, {'role': 'user', 'content': prompt}]
    ...
    def generate_json_schema(self, prompt: str, time_out: float = 10, temperature: float = 0, top_p: float = 1, max_tokens: int = 512):
        code = self.generate(prompt, time_out=time_out, temperature=temperature, top_p=top_p, max_tokens=max_tokens)
        return code

    def process_generation_to_code(self, gens: str):
        if '```json' in gens:
            gens = gens.split('```json')[1].split('```')[0]
        return gens.split('\n')
    
    def run(self, prompt: str, time_out: float = 10, temperature: float = 0.0, top_p: float = 1.0, max_tokens: int = 512):
        code = self.generate_json_schema(prompt, temperature=temperature, top_p=top_p, max_tokens=max_tokens)
       # with timeout(time_out):
       #     try:
       #         exec_result = self.execute(code)
       #     except Exception as e:
